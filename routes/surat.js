...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype !== 'application/pdf') return cb(new Error('Hanya file PDF yang diperbolehkan!'));
    cb(null, true);
  }
});
...
router.post('/surat/tambah', ensureLogin, upload.single('file_pdf'), async (req, res) => {
  const { nomor_surat, hal, dari, ditujukan_kepada, tanggal_surat, tanggal_diterima, catatan, jenis_surat, sifat } = req.body;
...
// ===================== PROSES EDIT =====================
router.post('/surat/edit/:id', ensureLogin, upload.single('file_pdf'), async (req, res) => {
  const { nomor_surat, hal, dari, ditujukan_kepada, tanggal_surat, tanggal_diterima, catatan, jenis_surat, sifat } = req.body;
  ...
  try {
    ...
    if (oldData.rows.length === 0) return res.status(404).send('Surat tidak ditemukan');
    ...
    await pool.query(
      `UPDATE surat_masuk 
       ...
       WHERE id=$11`,
      [nomor_surat, hal, dari, ditujukan_kepada, tanggal_surat || null, tanggal_diterima || null, catatan, nama_file, jenis_surat, sifat, id]
    );
...
router.post('/surat/restore/:id', ensureLogin, async (req, res) => {
  try {
    await pool.query(
      'UPDATE surat_masuk SET deleted_at = NULL WHERE id = $1',
      [req.params.id]
    );
...
router.get('/surat/detail/:id', ensureLogin, async (req, res) => {
  try {
    ...
    // Cek data surat
    const result = await pool.query(
      'SELECT * FROM surat_masuk WHERE id = $1',
      [suratId]
    );
